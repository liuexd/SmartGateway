 ...
 *
 * 职责：
 *   1. 周期上报DATA数据帧（T/H/P 等通用键值对）；
 *   2. 接收网关下发的CMD帧，按协议字典解释字段并模拟设备操作：
 *      - 无 LED 参数                    -> 回发NACK（unsupported_cmd）
 *      - LED 参数非法（非0/1）           -> 回发NACK（invalid_led）
 *      - 偶发故障（fail_every）          -> 每N条命令模拟一次硬件故障，回发NACK（sim_fault）
 *      - 参数合法且无故障                -> 模拟操作成功，更新LED状态，
 *                                          回发ACK（回显通用键值对）
 *
 * 底层串口I/O由调用方提供文件描述符。
 * =====================================================================
